...
class Measure():
    ...
    def ssim(self, imgA, imgB):
        ...
        if imgA.dtype == np.uint8:
            max_val = 255
        elif imgA.dtype == np.uint16:
            max_val = 1500
        score, diff = ssim(imgA, imgB, full=True, multichannel=True, data_range=max_val)
        return score
    ...
    def psnr(self, imgA, imgB):
        if imgA.dtype == np.uint8:
            ...
            max_val = 1500
        psnr_val = psnr(imgA, imgB, data_range=max_val)
        return psnr_val
...
def t(img):
    ...
    return to_tensor(to_4d(to_CHW(img))) 
...
def format_result(psnr, ssim, lpips):
    return f'{psnr:0.2f}, {ssim:0.4f}, {lpips:0.4f}'
