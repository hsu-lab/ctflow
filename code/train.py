            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    if hasattr(model, 'heats'):
                        for heat in model.heats:
                            for i in range(model.n_sample):
                                sr_img = util.tensor2img(visuals['SR', heat, i], out_type=np.uint16)  # uint16
                    ...
                    if not os.path.isfile(save_img_path_lq):
                        lq_img = util.tensor2img(visuals['LQ'], out_type=np.uint16)  # uint16
                        util.save_img(
                    ...
                    # calculate PSNR
                    crop_size = opt['scale']
                    gt_img = gt_img / 1500. * 255
                    sr_img = sr_img / 1500. * 255
