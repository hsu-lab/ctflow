...
def main():
    ...
    test_par_dir = opt_get(opt, ['result_path'], None)
    test_dir = os.path.join(test_par_dir, 'results', conf) if test_par_dir else os.path.join(this_dir, '..', 'results', conf)
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        if df is not None and len(df[(df['heat'] == heat) & (df['uid'] == idx_test)]) == 1:
            continue

        psnr = 0.0
        count = 0
        while psnr < 20. and count < 10:
            count += 1
            sr_t = model.get_sr(lq=lr_t.cuda(), heat=heat)

            sr = rgb(torch.clamp(sr_t, 0, 1), dtype='uint16')
            ...
            meas = OrderedDict(conf=conf, heat=heat, 
                            uid=os.path.basename(lr_path).split('_')[0], 
                            slice=os.path.basename(lr_path).split('_')[1].split('.')[0])
