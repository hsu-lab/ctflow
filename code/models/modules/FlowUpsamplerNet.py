...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        super().__init__()
        self.gt_size = opt_get(opt, ['datasets', 'train', 'GT_size'])
        ...
        self.H = H
        self.W = W
        self.scaleH = self.gt_size / H
        self.scaleW = self.gt_size / W
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None):
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            size = shape[2]
            level = int(np.log(self.gt_size / size) / np.log(2))
    ...
    def get_position_name(self, H, scale):
        downscale_factor = self.gt_size // H
